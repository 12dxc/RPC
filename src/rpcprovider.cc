...
void RpcProvider::notifyService(google::protobuf::Service *service)
{
    ...
    // 获取了服务对象的描述信息
    const google::protobuf::ServiceDescriptor *pservice_desc = service->GetDescriptor();
    // 获取服务的名字
    std::string service_name = pservice_desc->name();
    // 获取服务对象service的方法的数量
    int method_cnt = pservice_desc->method_count();
    ...
    for (int i = 0; i < method_cnt; ++i)
    {
        ...
        const google::protobuf::MethodDescriptor *pmethod_desc = pservice_desc->method(i);
        std::string method_name = pmethod_desc->name();
        service_info.method_map_.insert({method_name, pmethod_desc});
